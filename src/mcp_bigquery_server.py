...
class MCPBigQueryServer:
    ...
    async def _execute_query(self, arguments: Dict[str, Any]) -> CallToolResult:
        ...
        if not validate_sql_query(query):
            return CallToolResult(
                content=[TextContent(type="text", text="Error: Invalid or potentially unsafe SQL query")]
            )
    ...
    async def _validate_query(self, arguments: Dict[str, Any]) -> CallToolResult:
        ...
        try:
            ...
            if is_valid:
                ...
                return CallToolResult(
                    content=[TextContent(type="text", text=f"Query validation: {validation_result}")]
                )
            else:
                return CallToolResult(
                    content=[TextContent(type="text", text="Query validation failed: Invalid or unsafe query")]
                )
